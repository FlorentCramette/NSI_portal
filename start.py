...
def run_command(cmd, quiet=False):
    """Run command with optional output suppression to reduce Railway logs"""
    if quiet:
...
if __name__ == "__main__":
    # Quiet mode for all initialization commands to avoid Railway rate limit
    run_command("python manage.py migrate --noinput", quiet=True)

    if os.environ.get('DJANGO_SUPERUSER_USERNAME'):
        subprocess.run(
            ["python", "manage.py", "shell", "-c",
             "from accounts.models import User; username = '{}'; email = '{}'; password = '{}'; user, created = User.objects.get_or_create(username=username, defaults={{'email': email, 'is_student': False, 'is_teacher': True}}); user.set_password(password); user.is_superuser = True; user.is_staff = True; user.is_student = False; user.is_teacher = True; user.save()".format(
                 os.environ.get('DJANGO_SUPERUSER_USERNAME'),
                 os.environ.get('DJANGO_SUPERUSER_EMAIL'),
                 os.environ.get('DJANGO_SUPERUSER_PASSWORD'))],
            capture_output=True, text=True)

    # All content population commands in quiet mode
    ...
    run_command("python manage.py collectstatic --noinput --clear", quiet=True)
    ...
    # Single startup log instead of multiple prints
    print(f"Starting Gunicorn: {workers}w/{threads}t on port {port}")
